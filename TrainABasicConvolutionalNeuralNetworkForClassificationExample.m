...
batchsize= 4;
imds = imageDatastore('svdnetwork', 'IncludeSubfolders',true,'ReadSize',batchsize,'LabelSource','foldernames','FileExtensions','.nii','ReadFcn',@mycustomreader);
%% 
...
myactivations = zeros(length(imds.Labels),1);
...
options = trainingOptions('adam', ...
    ...
    'OutputNetwork','best-validation',...'best-validation' 'last-iteration'
...
myactivations(cv.test(iii)) =  outputfc(1,1,1,1,:);
...
figure(4)
histogram(myactivations(imds.Labels=='0'),20)
hold on
histogram(myactivations(imds.Labels=='1'),20)
%% 
% _Copyright 2018 The MathWorks, Inc._
function imagedata= mycustomreader(filename)
     vectorimage = squeeze(niftiread(filename));
     imagedata = vectorimage(:,:,:,1).* (vectorimage(:,:,:,2) >0);
end
