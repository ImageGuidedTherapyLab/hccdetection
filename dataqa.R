...
server <- function(input, output) {
  ...
  observeEvent(input$table_rows_selected, 
      ...
      system(paste0('echo ',my.data$uid[input$table_rows_selected],';vglrun itksnap -g ',my.data$image[input$table_rows_selected],' -s ', my.data$label[input$table_rows_selected],' -l labelkey.txt'),wait = T)
    )
}
