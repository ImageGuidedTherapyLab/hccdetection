...
# source('autoepmstats.R')
fname <- "autostats/widejoin.csv" 
rawdataset <- read.csv(fname, na.strings=c(".", "NA", "", "?"), strip.white=TRUE, encoding="UTF-8")
#benigndataset <- read.csv("autostats/widejoin.csv" , na.strings=c(".", "NA", "", "?"), strip.white=TRUE, encoding="UTF-8")
...
#benignsubset  <- subset(benigndataset ,                                               LabelID < 6 & FeatureID == 'epm')
benignsubset  <- subset(rawdataset ,      Status == 'control' & LabelID > 0  &   LabelID < 6 & FeatureID == 'epm')
benignsubsetwithqa = cbind(benignsubset, QA='Usable')
benignsubsetwithqa$LabelID = 7
epmbenignPreDxCntrl = rbind(epmdataPreDxCntrl, benignsubsetwithqa )
boxplot(Mean~Status+LabelID,data=epmbenignPreDxCntrl , main="Pre Dx vs Control", xlab="LI-RADS", ylab="EPM RMSD",  ylim = c(0, 1.5)) ; text(5.5, .5, 'background');
boxplot(Mean~Status+LabelID,data=epmbenignPreDxCntrl , main="Pre Dx vs Control", xlab="LI-RADS", ylab="EPM RMSD", names=c("LR3","","LR4","","Case","Cntl","","Benign"), ylim = c(0, 1.5)) ; text(5.5, .5, 'background');
