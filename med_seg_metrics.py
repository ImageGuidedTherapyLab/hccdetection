...
def SurfaceHausdorff(truth, pred, mode):
    ...
    if mode == 'mean':
        hausdorffSurface = np.max(np.mean(pred2truth_distances), np.mean(truth2pred_distances))
    elif mode == '95':
        hausdorffSurface = np.max(np.percentile(pred2truth_distances, 95), np.percentile(truth2pred_distances, 95))
    elif mode == 'max':
        hausdorffSurface = np.max(np.max(pred2truth_distances), np.max(truth2pred_distances))
...
def HausdorffDistance(truth, pred, mode):
    ...
    if mode == 'mean':
        hausdorffDistance = np.max(np.mean(pred2truth_distances), np.mean(truth2pred_distances))
    elif mode == '95':
