...
if trainedNN.net.Layers(1).InputSize(3) == 1
  ...
  disp('3D')
  [tempSeg,scores,allscores] = semanticseg(niivolume ,trainedNN.net,'ExecutionEnvironment',ExecutionEnvironment, 'outputtype', 'uint8');
end
...
%% write output to disk as a nifti file
outputlabel = fullfile(outputpath,'score')
infoout = info;
infoout.Filename = outputlabel;
infoout.Datatype = 'single';
niftiwrite(allscores(:,:,:,2) ,outputlabel ,infoout,'Compressed',true)
end
